...
export const solCnx = new solW3.Connection(
  'https://api.mainnet-beta.solana.com'
) // https://solana-api.projectserum.com //https://api.mainnet-beta.solana.com  https://solapeswap.rpcpool.com/

const pendingInits = new Map()
export async function getMintForTokenAccount(
  ...
  return (
    (tokenInfo.value?.data as solW3.ParsedAccountData).parsed?.info?.mint ?? ''
  )
}
export async function getBalanceForTokenAccount(
  tokenAcc: TokenAccount
): Promise<number> {
  return await solCnx.getBalance(tokenAcc.publicKey)
}
export async function getTokenInfoForTokenAccount(
  tokenAcc: TokenAccount
...
export async function initSol(addressObj: Address) {
  ...
  const tokenAccounts = await solCnx.getTokenAccountsByOwner(
    addressObj.publicKey,
    { programId: TOKEN_PROGRAM_ID }
  ) //, {limit:50}
  const tokenAccountObjs: TokenAccount[] = tokenAccounts.value.map(
    (eachAccount) =>
      new TokenAccount({
  ...
  console.log(
    `tokens accounts for ${addressObj.short}`,
    tokenAccountObjs,
    tokenAccounts
  )
  ...
  if (addressObj.status === AddressStatus.Active) {
    ...
    if (tokenAccountObjs?.length) {
      ...
      for (const eachAcc of tokenAccountObjs) {
        await Timeout.set(ta++ * 800) // throttle

        const balResponse = await solCnx.getTokenAccountBalance(
          eachAcc.publicKey
        )
...
export async function getSigArrayForTokenAccount(
  address58: string,
  limit = 10
) {
...
export async function getTxsForAddress(address58: string, limit = 20) {
  ...
  console.log(
    `fetching ${sigArray.length} (limit: ${limit}) sigs took ${
      performance.now() - stSigs
    }ms`
  )
  ...
  console.log(
    `fetching ${cacheHits?.length} cached txs took ${
      performance.now() - stCache
    }ms`
  )
  ...
  if (uncachedCount) {
    ...
    for (let sl = 0; sl <= sigArrayUncached.length; sl += 10) {
      ...
      const moreTxs = (
        await solCnx.getParsedConfirmedTransactions(sigArrayFetch)
      ).filter((tx) => !!tx)
      if (moreTxs) {
        ...
        moreTxs.forEach(async (eachTx, i) => {
          ...
          // eachTx?.transaction.message.instructions.forEach(
          //   (eachInst, insti) => {
          ...
          //         console.log(eachInst.decData)
          //         if (eachInst.decData.hasOwnProperty('newOrderV3')) {
          //           const newInst = new NewOrderInstruction(
          //             eachInst.decData.newOrderV3
          //           )
          //           console.log(newInst)
          //           eachInst.info.newOrder = newInst
          //           eachInst.info.orderInfo = newInst.short
          //         }
          //       } catch (e) {
          //         console.log(e)
          //       }
          //       console.log(
          //         `serum instruction for tx ${i} (${shortSig}) inst[${insti}]`,
          //         eachInst
          //       )
          //     }
          //   }
        ...
        txsArray = txsArray.concat(
          moreTxs as solW3.ParsedConfirmedTransaction[]
        )
      }
    }
    console.groupEnd()
    console.log(
      `fetching ${uncachedCount} txs took ${performance.now() - st}ms`,
      txsArray
    )
  }
  const txs = uncachedCount
    ? await offlineDB.Transactions.bulkGet(sigArray)
    : cachedTxs
